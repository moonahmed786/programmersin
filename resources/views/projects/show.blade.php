...
<header class="mb-12 flex items-end justify-between">
    <div class="space-y-4">
        <div class="flex items-center gap-4">
            <a href="{{ url()->previous() }}" class="w-10 h-10 rounded-xl bg-surface-container flex items-center justify-center text-on-surface-variant hover:text-primary transition-all border border-outline-variant/10 shadow-sm">
    ...
    <div class="flex gap-4">
        <button class="px-3 py-3 bg-surface-container-lowest text-on-surface rounded-xl border border-outline-variant/20 hover:bg-surface-container transition-all group">
            <span class="material-symbols-outlined text-sm group-hover:scale-110 transition-transform">edit</span>
        </button>
        <button class="px-6 py-3 bg-on-surface text-surface rounded-xl font-black text-sm uppercase tracking-widest shadow-2xl shadow-on-surface/30 transition-all hover:-translate-y-1">Archive</button>
    </div>
    @endif
</header>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 space-y-10 text-sm">
        <!-- Milestone Progress -->
        <section class="bg-surface-container-lowest rounded-3xl p-10 border border-outline-variant/10 shadow-2xl shadow-slate-200/40 space-y-8">
            ...
            <div class="w-full h-3 bg-surface-container rounded-full overflow-hidden shadow-inner relative">
                <div class="h-full bg-primary rounded-full shadow-[0_0_12px_rgba(0,86,210,0.3)] transition-all duration-1000 ease-out" style="width: 45%;"></div>
            </div>
        ...
        <section class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="bg-surface-container-lowest p-6 rounded-2xl border border-outline-variant/10 flex items-center gap-4 group hover:shadow-lg transition-all">
                    <div class="w-12 h-12 rounded-xl bg-surface-container flex items-center justify-center overflow-hidden border border-outline-variant/10 shadow-inner">
                ...
                <div class="col-span-full py-10 bg-surface-container-low/30 rounded-3xl border-2 border-dashed border-outline-variant/20 flex flex-col items-center gap-3">
    ...
    <div class="space-y-8">
            ...
            <div class="space-y-8">
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <span class="font-black font-mono tracking-tighter text-on-surface text-lg text-emerald-600">${{ number_format($project->budget, 2) }}</span>
                    </div>
                </div>
        ...
        <!-- Service Badge -->
        <div class="bg-primary p-8 rounded-3xl text-on-primary shadow-2xl shadow-primary/20 group relative overflow-hidden">
            <div class="relative z-10 space-y-4">
                <span class="text-[10px] bg-white/20 px-3 py-1 rounded-full font-black uppercase tracking-widest inline-block border border-white/20">Core Service Offering</span>
                ...
                <div class="pt-4">
                    <button class="w-full py-4 bg-white/10 hover:bg-white/20 border border-white/20 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all">Service Details</button>
                </div>
            </div>
