...
<!-- Navigation Node Initialization Header -->
<div class="mb-14">
    <div class="flex items-center gap-6 mb-4">
        ...
        <div>
            <h1 class="text-3xl font-black tracking-tighter text-slate-900 uppercase italic">
                <span class="text-gradient">Initialize Link Node</span>
            </h1>
            <p class="text-[10px] text-slate-400 font-extrabold uppercase tracking-[0.3em] mt-2">Initializing new architectural navigation node</p>
        </div>
    </div>
</div>
    ...
    <div class="glass-surface rounded-stellar overflow-hidden border border-white/80">
        <form action="{{ route('admin.menus.store') }}" method="POST">
            ...
            <div class="p-12 space-y-14">
                <!-- Group 01: Core Link Logic -->
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    <div>
                        <span class="text-[10px] font-black text-primary uppercase tracking-[0.3em] block mb-2">PHASE 01</span>
                        <h3 class="font-black text-slate-900 tracking-tight uppercase text-sm italic">Core Link Logic</h3>
                    </div>
                    <div class="lg:col-span-2 space-y-10">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1" for="title">Display Label</label>
                                <input id="title" type="text" name="title" required value="{{ old('title') }}"
                                    ...
                                    placeholder="LABEL_CONSTANT">
                                @error('title') <p class="text-[9px] text-red-500 font-bold uppercase tracking-widest mt-2">{{ $message }}</p> @enderror
                            </div>

                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1" for="url">Destination URI Path</label>
                                <input id="url" type="text" name="url" value="{{ old('url') }}"
                                    class="w-full bg-slate-50/50 border border-slate-100 rounded-2xl px-6 py-4 text-xs font-mono font-bold text-primary focus:ring-4 focus:ring-primary/5 focus:border-primary/20 focus:bg-white transition-all italic shadow-inner"
                ...
                <!-- Group 02: Hierarchy & Location Registry -->
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    <div>
                        <span class="text-[10px] font-black text-tertiary uppercase tracking-[0.3em] block mb-2">PHASE 02</span>
                        <h3 class="font-black text-slate-900 tracking-tight uppercase text-sm italic">Hierarchy Registry</h3>
                    </div>
                    <div class="lg:col-span-2 space-y-10">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1" for="location">Menu Sector (Location)</label>
                                <div class="relative">
                                    <select id="location" name="location" required 
                                        ...
                                        <option value="header">HEADER_GLOBAL</option>
                                        <option value="footer">FOOTER_SITEMAP</option>
                                    </select>
                            ...
                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1" for="parent_id">Structural Parent</label>
                                <div class="relative">
                                    <select id="parent_id" name="parent_id" 
                                        class="w-full bg-slate-50/50 border border-slate-100 rounded-2xl px-6 py-4 text-[11px] font-black uppercase tracking-widest appearance-none focus:ring-4 focus:ring-primary/5 focus:border-primary/20 focus:bg-white transition-all">
                                        <option value="">-- NULL_PARENT (ROOT) --</option>
                                        @foreach($parentMenus as $parent)
                                            <option value="{{ $parent->id }}">{{ strtoupper($parent->title) }} ({{ strtoupper($parent->location) }})</option>
                                        @endforeach
                                    </select>
                                    <span class="material-symbols-outlined absolute right-6 top-1/2 -translate-y-1/2 text-slate-300 pointer-events-none text-lg">unfold_more</span>
                                </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1" for="order">Sequence Index (Order)</label>
                                <input id="order" type="number" name="order" value="{{ old('order', 0) }}"
                                    class="w-full bg-slate-50/50 border border-slate-100 rounded-2xl px-6 py-4 text-sm font-black text-slate-900 focus:ring-4 focus:ring-primary/5 focus:border-primary/20 focus:bg-white transition-all shadow-inner">
                            </div>
            ...
            <div class="px-12 py-10 bg-slate-50/50 border-t border-slate-100 flex items-center justify-between">
                ...
                <div class="flex items-center gap-6">
                    <a href="{{ route('admin.menus.index') }}" class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-300 hover:text-red-500 transition-colors">Abort Init</a>
