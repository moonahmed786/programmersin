...
<div class="max-w-2xl space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Edit Service</h1>
            <p class="text-sm text-slate-400 mt-1">Modify details for {{ $service->title }}</p>
        </div>
    </div>

    {{-- Form Card --}}
    <div class="bg-white dark:bg-slate-900 rounded-2xl border border-outline-variant/20 shadow-sm overflow-hidden">
        <form action="{{ route('admin.services.update', $service) }}" method="POST" class="divide-y divide-outline-variant/10">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 space-y-5">
                ...
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                    <div class="space-y-1.5">
                        ...
                        <div class="relative">
                            <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-lg">{{ old('icon', $service->icon) ?? 'category' }}</span>
